...
keys = [Key(key[0], key[1], *key[2:]) for key in [
    ...
    #9
    ([mod], "KP_Page_Up",lazy.spawn("firefox instagram.com")),
    
    # ------------ SISTEMA ------------ 
    

    ([mod, "control"], "F3", lazy.spawn("reboot")),
    ([mod, "control"], "F4", lazy.spawn("poweroff")),
    ([], "Print", lazy.spawn("scrot -e  'mv $f /home/luis/.screenshots/'")),
    
    # ------------ VOLUMEN ------------ 
    
    ([mod], "KP_Subtract", lazy.spawn("pamixer --decrease 5")),
    ([mod], "KP_Add", lazy.spawn("pamixer --increase 5")),
    ([mod], "KP_Multiply", lazy.spawn("pamixer --toggle-mute")),

    # ------------ VOLUMEN ------------ 
    ([], "Pause", lazy.spawn("playerctl play-pause")),
